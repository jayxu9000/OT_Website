...
function NewList() {
    ...
    useEffect(() => {
        const fetchNonVerifiedUsers = async () => {
            try {
                const response = await fetch('http://localhost:5000/users/nonVerifiedUsers'); // Adjust the URL as needed
                if (response.ok) {
    ...
    const handleVerification = async (userId) => {
        ...
        try {
            const response = await fetch(`http://localhost:5000/users/promoteToVerified/${userId}`, {
                method: 'PUT',
                headers: {
